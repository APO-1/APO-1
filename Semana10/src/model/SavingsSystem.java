...
public class SavingsSystem {
	...
	public int findUser(String ID) {
		boolean found = false;
		int position = -1;
		int cantidadARecorrer = (getFirstValidPosition() == -1) ? 10 : getFirstValidPosition();
		for (int i = 0; i < cantidadARecorrer && !found; i++) {
	...
	public String addSaving(String nameSaving, double costSaving, int category, String userID,
		...
		if (users[0] != null && findUser(userID) != -1) {
			...
			msg = users[findUser(userID)].addSaving(saving);
		}
		return msg;
	}
	 ...
	 * 
	 * @return pos -1 if the position does not exist, a number in [0, 9] if exist a
	 *         valid position
	 */
